...
class DirectoryArchive:
    def __init__(self, fullpath, format):  # noqa: A002
...
def _upload_blob(file_path):
    ...
    try:
        requests.put(response["createUrl"], data=file_data, headers=headers, timeout=10)
...
def upload_files(folder, quiet=False, dir_mode="skip"):  # noqa: FBT002
    ...
    for file_name in os.listdir(folder):
        if file_name in [
            ...
            continue
        tokens.append(_upload_file_or_folder(folder, file_name, dir_mode, quiet))
    return tokens


def _upload_file_or_folder(parent_path, file_or_folder_name, dir_mode, quiet=False):  # noqa: FBT002
    ...
    if os.path.isfile(full_path):
        ...
        if dir_mode in ["zip", "tar"]:
            with DirectoryArchive(full_path, dir_mode) as archive:
                return _upload_file(archive.name, archive.path, quiet)
        elif not quiet:
            logger.info("Skipping folder: " + file_or_folder_name + "; use '--dir-mode' to upload folders")
    elif not quiet:
