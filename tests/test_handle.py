...
class TestHandle(BaseTestCase):
    ...
    def test_invalid_dataset_handle_format(self) -> None:
        with self.assertRaises(ValueError):
            parse_dataset_handle("just-one-part")

    def test_invalid_version_dataset_handle(self) -> None:
        with self.assertRaises(ValueError):
            parse_dataset_handle("owner/dataset/not-a-number")
