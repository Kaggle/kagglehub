...
class TestKaggleApiV1Client(BaseTestCase):
    ...
    async def test_download_with_integrity_check(self) -> None:
    ...
    async def test_resumable_download_with_integrity_check(self) -> None:
        with TemporaryDirectory() as d:
            out_file = os.path.join(d, "out")
    ...
    async def test_download_no_integrity_check(self) -> None:
        with TemporaryDirectory() as d:
            out_file = os.path.join(d, "out")
    ...
    async def test_download_corrupted_file_fail_integrity_check(self) -> None:
        with TemporaryDirectory() as d:
            out_file = os.path.join(d, "out")
