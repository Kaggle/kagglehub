...
class TestKaggleCacheModelDownload(BaseTestCase):
    ...
    async def test_kaggle_resolver_skipped(self) -> None:
    ...
    async def test_versioned_model_download_with_force_download(self) -> None:
        with stub.create_env():
            model_path = await kagglehub.model_download(VERSIONED_MODEL_HANDLE)
            model_path_forced = await kagglehub.model_download(VERSIONED_MODEL_HANDLE, force_download=True)
    ...
    async def test_versioned_model_download_with_force_download_explicitly_false(self) -> None:
        with stub.create_env():
