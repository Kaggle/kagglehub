...
class FakeImpl:
    ...
    async def is_supported(self, *args: Any, **kwargs: Any) -> bool:  # noqa: ANN401
...
class RegistryTest(BaseTestCase):
    async def test_calls_only_supported(self) -> None:
        ...
        val = await r(SOME_VALUE)

        self.assertEqual(SOME_VALUE, val)

    async def test_calls_first_supported_reverse(self) -> None:
        r = registry.MultiImplRegistry("test")
    ...
    async def test_calls_throw_not_supported(self) -> None:
        r = registry.MultiImplRegistry("test")
        r.add_implementation(FakeImpl(lambda _: False, fail_fn))

        with self.assertRaisesRegex(RuntimeError, r"Missing implementation"):
            await r(SOME_VALUE)
